...
export class SignalProtocolStore extends EventEmitter {
  ...
  readonly #dataWriter: ClientWritableInterface;
  ...
  async hydrateCaches(): Promise<void> {
    await Promise.all([
      (async () => {
        ...
        const map = (await this.#dataReader.getItemById(
          'identityKeyMap'
      ...
      (async () => {
        ...
        const triples = await this.#dataReader.getAllKyberTriples();
      ...
      _fillCaches<string, IdentityKeyType, PublicKey>(
        this,
        'identityKeys',
        this.#dataReader.getAllIdentityKeys()
      ),
  ...
  async confirmKyberPreKey(
    ...
    await this.#dataWriter.createOrUpdateKyberPreKey(confirmedItem.fromDB);
  ...
  async storeKyberPreKeys(
    ...
    await this.#dataWriter.bulkAddKyberPreKeys(toSave);
  ...
  async clearKyberPreKeyStore(): Promise<void> {
    ...
    const changes = await this.#dataWriter.removeAllKyberPreKeys();
  ...
  async storePreKeys(
    ...
    await this.#dataWriter.bulkAddPreKeys(toSave);
  ...
  async clearPreKeyStore(): Promise<void> {
    ...
    const changes = await this.#dataWriter.removeAllPreKeys();
  ...
  async confirmSignedPreKey(
    ...
    await this.#dataWriter.createOrUpdateSignedPreKey(confirmedItem.fromDB);
  ...
  async storeSignedPreKey(
    ...
    await this.#dataWriter.createOrUpdateSignedPreKey(fromDB);
    this.signedPreKeys.set(id, {
      hydrated: false,
      fromDB,
    });
  ...
  async removeSignedPreKeys(
    ...
    await this.#dataWriter.removeSignedPreKeyById(ids);
    ids.forEach(id => {
  ...
  async clearSignedPreKeysStore(): Promise<void> {
    ...
    const changes = await this.#dataWriter.removeAllSignedPreKeys();
  ...
  async removeSenderKey(
    ...
    try {
      ...
      await this.#dataWriter.removeSenderKeyById(id);
  ...
  async removeAllSenderKeys(): Promise<void> {
    return this.withZone(GLOBAL_ZONE, 'removeAllSenderKeys', async () => {
      ...
      await this.#dataWriter.removeAllSenderKeys();
    });
  }
  ...
  async #commitZoneChanges(name: string): Promise<void> {
    ...
    await this.#dataWriter.commitDecryptResult({
  ...
  async storeSession(
    ...
    await this.withZone(zone, 'storeSession', async () => {
      ...
      const conversation = this.#conversationController.lookupOrCreate({
        serviceId,
        ...
        > = await Promise.all(
          entries.map(async entry => {
            if (entry.hydrated) {
              ...
              if (record.hasCurrentState(this.#getRequirePqRatio())) {
                return { record, entry };
              }
  ...
  async removeSession(qualifiedAddress: QualifiedAddress): Promise<void> {
    return this.withZone(GLOBAL_ZONE, 'removeSession', async () => {
      ...
      try {
        await this.#dataWriter.removeSessionById(id);
  ...
  async removeSessionsByConversation(identifier: string): Promise<void> {
    return this.withZone(
      ...
      async () => {
        ...
        const id = this.#conversationController.getConversationId(identifier);
        strictAssert(
          id,
        ...
        await this.#dataWriter.removeSessionsByConversation(id);
      }
    );
  }
  ...
  async removeSessionsByServiceId(serviceId: ServiceIdString): Promise<void> {
    return this.withZone(GLOBAL_ZONE, 'removeSessionsByServiceId', async () => {
      ...
      await this.#dataWriter.removeSessionsByServiceId(serviceId);
    });
  }
  ...
  async #_archiveSession(entry?: SessionCacheEntry, zone?: Zone) {
    ...
    await this.enqueueSessionJob(
      ...
      async () => {
        ...
        if (!item.hasCurrentState(this.#getRequirePqRatio())) {
          return;
        }
  ...
  async clearSessionStore(): Promise<void> {
    return this.withZone(GLOBAL_ZONE, 'clearSessionStore', async () => {
      ...
      this.#pendingSessions.clear();
      const changes = await this.#dataWriter.removeAllSessions();
  ...
  async lightSessionReset(qualifiedAddress: QualifiedAddress): Promise<void> {
    ...
    const sessionResets = this.#itemStorage.get(
      'sessionResets',
    ...
    await this.#itemStorage.put('sessionResets', sessionResets);
  ...
  async getOrMigrateIdentityRecord(
    ...
    const conversation = this.#conversationController.get(newId);
    if (!conversation) {
      return undefined;
    }
    ...
    const changes = await this.#dataWriter.removeIdentityKeyById(
      record.fromDB.id
    );
  ...
  async isTrustedIdentity(
    ...
    const isOurIdentifier = this.#itemStorage.user.isOurServiceId(
  ...
  isTrustedForSending(
    ...
    if (!identityRecord) {
      ...
      //   conversation.
      const conversation = this.#conversationController.get(serviceId);
  ...
  async #_saveIdentityKey(data: IdentityKeyType): Promise<void> {
    ...
    await this.#dataWriter.createOrUpdateIdentityKey(data);
  ...
  async #saveIdentityWithAttributesOnQueue(
    ...
    const serviceIdKind = this.#itemStorage.user.getOurServiceIdKind(serviceId);
    if (serviceIdKind !== ServiceIdKind.PNI) {
      this.#conversationController.getOrCreate(id, 'private');
    }
  ...
  checkPreviousKey(
    ...
    if (conversation && previousIdentityKeyBase64) {
      ...
      try {
        ...
        drop(this.#dataWriter.updateConversation(conversation.attributes));
      } catch (error) {
        log.error(
  ...
  async removeIdentityKey(serviceId: ServiceIdString): Promise<void> {
    ...
    await this.#dataWriter.removeIdentityKeyById(serviceId);
  ...
  getUnprocessedByIdsAndIncrementAttempts(
    ...
    return this.withZone(
      ...
      async () => {
        return this.#dataWriter.getUnprocessedByIdsAndIncrementAttempts(ids);
      }
    );
  }
  ...
  removeUnprocessed(idOrArray: string | Array<string>): Promise<void> {
    return this.withZone(GLOBAL_ZONE, 'removeUnprocessed', async () => {
      await this.#dataWriter.removeUnprocessed(idOrArray);
    });
  }
  ...
  removeAllUnprocessed(): Promise<void> {
    ...
    return this.withZone(GLOBAL_ZONE, 'removeAllUnprocessed', async () => {
      await this.#dataWriter.removeAllUnprocessed();
    });
  }
  ...
  async removeOurOldPni(oldPni: PniString): Promise<void> {
    ...
    await Promise.all([
      this.#itemStorage.put(
        'identityKeyMap',
        omit(this.#itemStorage.get('identityKeyMap') || {}, oldPni)
      ),
      this.#itemStorage.put(
        'registrationIdMap',
        omit(this.#itemStorage.get('registrationIdMap') || {}, oldPni)
      ),
      this.#dataWriter.removePreKeysByServiceId(oldPni),
      this.#dataWriter.removeSignedPreKeysByServiceId(oldPni),
      this.#dataWriter.removeKyberPreKeysByServiceId(oldPni),
    ]);
  }
  ...
  async updateOurPniKeyMaterial(
    ...
    await Promise.all<void>([
      this.#itemStorage.put('identityKeyMap', {
        ...this.#itemStorage.get('identityKeyMap'),
        [pni]: {
          pubKey: pniPublicKey,
          privKey: pniPrivateKey,
        },
      }),
      this.#itemStorage.put('registrationIdMap', {
        ...this.#itemStorage.get('registrationIdMap'),
        [pni]: registrationId,
      }),
      (async () => {
        const newId = Math.max(1, wrappingAdd24(signedPreKey.id(), 1));
        log.warn(`${logId}: Updating next signed pre key id to ${newId}`);
        await this.#itemStorage.put(
          SIGNED_PRE_KEY_ID_KEY[ServiceIdKind.PNI],
          newId
        );
      })(),
      ...
      (async () => {
        ...
        await this.#itemStorage.put(KYBER_KEY_ID_KEY[ServiceIdKind.PNI], newId);
      })(),
  ...
  async removeAllData(): Promise<void> {
    await this.#dataWriter.removeAll();
    await this.hydrateCaches();

    this.#itemStorage.reset();
    await this.#itemStorage.fetch();

    this.#conversationController.reset();
    await this.#conversationController.load();
  ...
  async removeAllConfiguration(isPrimary: boolean): Promise<void> {
    ...
    this.#conversationController.getAll().forEach(conversation => {
      conversation.set({
        storageID: undefined,
    ...
    await this.#dataWriter.removeAllConfiguration(isPrimary);
  ...
  signAlternateIdentity(): PniSignatureMessageType | undefined {
    const ourAci = this.#itemStorage.user.getCheckedAci();
    const ourPni = this.#itemStorage.user.getPni();
    if (!ourPni) {
  ...
  getLastDistinguishedTreeHead(): Uint8Array<ArrayBuffer> | null {
    return this.#itemStorage.get('lastDistinguishedTreeHead') ?? null;
  }
  ...
  async setLastDistinguishedTreeHead(
    ...
    if (bytes == null) {
      await this.#itemStorage.remove('lastDistinguishedTreeHead');
    } else {
      await this.#itemStorage.put('lastDistinguishedTreeHead', bytes);
    }
  }
  ...
  async getKTAccountData(
    ...
    const data = await this.#dataReader.getKTAccountData(aci);
    return data ?? null;
  }
  ...
  async setKTAccountData(
    ...
    const aci = fromAciObject(aciObject);
    return this.#dataWriter.setKTAccountData(aci, bytes);
  }
